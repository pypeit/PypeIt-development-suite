...
def test_shane_hamspec(redux_out):
    """
    Check the Shane/Hamspec wavelength calibration for both detector eras:
    enough echelle orders solved and per-order RMS under control.  The
    passing dev-suite runs give 19 orders @ median 0.15 px (Hamilton,
    Loral era) and 64 orders @ median 0.28 px (Hamilton_e2v).

    Generated by JXP and Claude.
    """
    for setup, min_orders, med_rms in zip(['Hamilton', 'Hamilton_e2v'],
                                          [15, 50],
                                          [0.25, 0.40]):
        this_calib = Path(redux_out).absolute() / 'shane_hamspec' / setup \
            / 'Calibrations'
        assert this_calib.is_dir(), \
            f'Calibration directory {this_calib} does not exist!'
        file_path = list(this_calib.glob('WaveCalib_*.fits'))[0]
        waveCalib = WaveCalib.from_file(file_path)
        rms_vals = np.array([ww.rms for ww in waveCalib.wv_fits
                             if ww is not None and ww.rms is not None])
        # Enough orders must have a wavelength solution
        assert rms_vals.size >= min_orders, \
            f'Only {rms_vals.size} orders solved for {setup}!'
        # Typical quality
        assert np.median(rms_vals) <= med_rms, \
            f'Median wave RMS for {setup} is too high!'
        # No order may exceed the (raised, Q&A 51) ~0.25xFWHM acceptance gate
        assert np.all(rms_vals <= 0.80), f'Wave RMS for {setup} is too high!'
