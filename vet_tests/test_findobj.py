...
# ESI Ech_1x1 reduced files used by all tests
ESI_REDUX = os.path.join('keck_esi', 'Ech_1x1', 'Science')
...
def esi_data(redux_out):
    ...
    - ``slit_righ``:   tweaked right slit edges (nspec, norders)
    ...
    """
    sci_dir = os.path.join(redux_out, ESI_REDUX)

    # --- spec1d: one SpecObj per order for ECH_OBJID==1 ---
    sobjs_all = specobjs.SpecObjs.from_fitsfile(os.path.join(sci_dir, SPEC1D_FILE))
    ...
    s2d = spec2dobj.Spec2DObj.from_file(os.path.join(sci_dir, SPEC2D_FILE), 'DET01')
    slits = s2d.slits
    ...
    slit_righ = slits.right_tweak if slits.right_tweak is not None else slits.right
...
def test_fill_in_orders_plate_scale_conversion(esi_data):
    ...
    sobjs_filled = findobj_skymask.ech_fill_in_orders(
        sobjs_partial, slit_left, slit_righ, slit_spat_id,
...
def test_ech_findobj_ineach_order_maxshift(esi_data):
    ...
    sobjs_tight = findobj_skymask.ech_findobj_ineach_order(
        ...
        esi_data['slit_left'], esi_data['slit_righ'], esi_data['slit_spat_id'],
