...
def test_shane_hamspec_era_archives():
    """
    Check the Shane/Hamspec detector-era identification and the era-matched
    selection of the echelle wavelength-calibration archives (Q&A 52): the
    Loral 2Kx2K (Hamilton) and e2v 4Kx4K (Hamilton_e2v) frames must map to
    their respective ``shane_hamspec_<era>_*`` reid_arxiv files.

    Generated by JXP and Claude.
    """
    raw = Path(os.environ['PYPEIT_DEV']).absolute() / 'RAW_DATA' \
        / 'shane_hamspec'
    spec = load_spectrograph('shane_hamspec')
    for setup, frame, detname, era in [
            ('Hamilton', 'd139.fits.gz', 'Loral2Kx2K', 'loral'),
            ('Hamilton_e2v', 'd2084.fits', 'e2v4Kx4K', 'e2v')]:
        example_file = raw / setup / frame
        assert example_file.is_file(), \
            f'Could not find example file {example_file}.'
        ...
        files = spec.get_echelle_angle_files(meta_dict={'detector': detector})
