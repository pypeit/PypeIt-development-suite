...
def test_hires():
    ...
    # Get the list of setup directories
    setups_names = ['lJ0306+1853_U074_RED_C2_ECH_0.72_XD_1.42_1x3',
    ...
    for setup in setups:
        # Find the relevant pypeit file constructed by hand.
        by_hand_pypeit = dev_root / 'pypeit_files' / f'keck_hires_{setup.name.lower()}.pypeit'
        ...
        # Run pypeit_setup
        ps = PypeItSetup.from_file_root(str(setup), 'keck_hires')
        ps.run(setup_only=True)
...
def test_shane_hamspec():
    """
    Check the automated frame typing of the Shane/Hamspec (Hamilton) frames
    against the by-hand dev-suite pypeit file.

    The Hamilton frames are typed from the aperture plate (``PLATENAM``):
    wide plates are pixelflats and narrow plates are trace/illum flats (see
    ``shane_hamspec.check_frame_type``), so this guards the plate-based
    typing for the Loral-era data.

    Generated by JXP and Claude.
    """
    dev_root = Path(os.getenv('PYPEIT_DEV')).absolute()
    assert dev_root.exists(), \
        f'PypeIt development suite directory does not exist: {dev_root}'

    # Both detector eras: Loral 2Kx2K (Hamilton) and e2v 4Kx4K (Hamilton_e2v)
    for setup_name in ['Hamilton', 'Hamilton_e2v']:
        setup = dev_root / 'RAW_DATA' / 'shane_hamspec' / setup_name
        assert setup.exists(), f'Raw data directory does not exist: {setup}'

        by_hand_pypeit = dev_root / 'pypeit_files' \
            / f'shane_hamspec_{setup.name.lower()}.pypeit'
        assert by_hand_pypeit.exists(), \
            f'By-hand pypeit file does not exist: {by_hand_pypeit}'

        # Set the output path and remove it if it already exists
        output_path = Path('.').absolute() / 'output'
        if output_path.exists():
            shutil.rmtree(output_path)

        # Run pypeit_setup
        ps = PypeItSetup.from_file_root(str(setup), 'shane_hamspec')
        ps.run(setup_only=True)

        # Write the automatically generated pypeit file
        pypeit_files = ps.fitstbl.write_pypeit(output_path,
                                               cfg_lines=ps.user_cfg)

        # Read the frame types from the by-hand and automated pypeit files
        byhand_pypeItFile = PypeItFile.from_file(by_hand_pypeit)
        auto_pypeItFile = PypeItFile.from_file(pypeit_files[0])

        # Check every by-hand frame type is reproduced (including the
        # standard: unlike science frames it is fully determined by the
        # headers here)
        for f in byhand_pypeItFile.frametypes.keys():
            type_list = np.sort(byhand_pypeItFile.frametypes[f].split(','))
            assert f in auto_pypeItFile.frametypes.keys(), \
                f'Frame {f} not automatically parsed for {setup_name}.'
            assert np.array_equal(
                type_list,
                np.sort(auto_pypeItFile.frametypes[f].split(','))), \
                f'Frame types differ for file {f} in {setup_name}\n' \
                f'    By-hand types: {byhand_pypeItFile.frametypes[f]}' \
                f'    Automated types: {auto_pypeItFile.frametypes[f]}'
