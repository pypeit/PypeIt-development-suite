...
blue_data = spec_hdu[10].data
red_data = spec_hdu[25].data
...
blue_smooth_flux, blue_smooth_ivar = ivarsmooth(blue_flux[-num:-3], blue_ivars[-num:-3], 5)
blue_smooth_flux = blue_smooth_flux.reshape((len(blue_flux[-num:-3]), 1))
blue_smooth_waves = blue_waves[-num:-3].reshape((len(blue_waves[-num:-3]), 1))
...
plt.plot(blue_smooth_waves[:,0],regr.predict(blue_smooth_waves),'r')
plt.show()
'''
