...
def get_sens_from_file(std1dfile=None, instrument='GNIRS', star_type=None, star_mag=None,star_ra=None,
                       star_dec=None, sens_polyorder=8, mask_abs_lines=True, disp=True, show=True, debug=False):
    ...
    if (instrument=='GNIRS') or (instrument=='NIRES') or (instrument == 'MOSFIRE'):
    ...
    TelSens = telluric.sensfunc_telluric(std1dfile, telgridfile, sensfile, star_type=star_type, star_mag=star_mag,
                                         ...
                                         polyorder=sens_polyorder, disp=show, debug=debug)
...
def apply_tell_from_file(z_obj, stackfilename, tell_method='qso', instrument='NIRES', telgridfile=None,
    ...
    if tell_method=='qso':
        ...
        TelQSO = telluric.qso_telluric(stackfilename, telgridfile, pca_file, z_obj, telloutfile, outfile,
                                       create_bal_mask=None, disp=disp, debug=debug, show=show)
...
# This will be a function that fluxes, coadds, and telluric corrects only. Sensitivity function should be created by a separate
# method which is run by a script
def flux_coadd_telluric(spec1dfiles, sensfile, tell_function, telloutfile, outfile, objids=None,z_qso=None, ex_value='OPT',
                        polyorder=3, fit_region_min=[9200.0], fit_region_max=[9700.0], scale_method=None, hand_scale=None,
                        const_weights=False, wave_grid_min=None, wave_grid_max=None, mask_lyman_a=True, do_flux=False, do_stack=False, do_tell=False, use_exist_sens=True,
    ...
    if do_stack:
        ...
        if pypeline == 'Echelle':
            wave_stack, flux_stack, ivar_stack, mask_stack = coadd1d.ech_combspec(fnames, objids, show=show,
            ...
            wave_stack, flux_stack, ivar_stack, mask_stack = coadd1d.multi_combspec(fnames, objids, show=show,
    ...
    if do_tell:
        apply_tell_from_file(z_qso, stackfile, tell_method=tell_method, instrument=instrument, telgridfile=telgridfile,
                             ...
                             mask_lyman_a=mask_lyman_a, show=show, debug=debug)
...
# This will be a script that fluxes, coadds, and telluric corrects only. Sensitivity function should be created by a separate
# method which is run by a script
