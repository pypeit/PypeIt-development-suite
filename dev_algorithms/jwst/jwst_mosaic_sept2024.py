...
from jwst_targets_sept2024 import jwst_targets
...
bkg_redux = True #False #False #False #False
#run_stage1 = True
#run_stage2 = True
...
pypeit_output_dir = os.path.join(redux_dir, 'pypeit2')
...
# Copy msa files to output_dir
for msa in msa_list:
    if os.path.isfile(msa) and not os.path.isfile(os.path.join(output_dir, os.path.basename(msa))):
        msgs.info('Copying MSA file: {0}'.format(msa))
        os.system('cp {0} {1}'.format(msa, output_dir))
