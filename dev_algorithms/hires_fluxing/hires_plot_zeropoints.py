...
# for plots
mcolors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan']
plt.rc('font', family='serif')
plt.rcParams['mathtext.default'] = u'regular'
plt.rc('axes', labelsize=15)
plt.rc('legend', fontsize=9, borderpad=1., handlelength=1., handleheight=1., labelspacing=0.5, handletextpad=0.5,
            ...
            for sens in sens_tab:
                if sens['WAVE_MIN'] > _wmin and sens['WAVE_MAX'] <= _wmax:
                    ...
                    axis.plot(wave[wv_gpm], zeropoint_data[wv_gpm], drawstyle='steps-mid',
                              color=mcolors[i], linewidth=0.6, alpha=0.4, zorder=5)
                    if nplot == 0:
                        axis.plot(wave[wv_gpm], zeropoint_fit[wv_gpm], label=f'{stdname} - {datasets[i]}',
                                  color=mcolors[i], linewidth=1.3, alpha=1., zorder=0)
                    else:
                        axis.plot(wave[wv_gpm], zeropoint_fit[wv_gpm], color=mcolors[i], linewidth=2.0, alpha=1., zorder=0)
