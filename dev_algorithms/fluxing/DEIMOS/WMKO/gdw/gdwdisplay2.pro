...
;	FILL:		scale image to fit display window [def='yes']?
;	ZSCALE:		display range of greylevels near median [def='yes']?
;	CONTRAST:	contrast adjustment for zscale algorithm [def=0.25]
;	ZRANGE:		display full image intensity range [def='yes']?
;	Z1:		minimum greylevel to be displayed
;	Z2:		maximum greylevel to be displayed
;	ZTRANS:		greylevel transformation (linear|log) [def='linear']
;
; OUTPUTS:
;
; SIDE EFFECTS:
...
if( not keyword_set(XSIZE)) then ysize=def_size
