;------------------------------------------------------------------------
pro deimos_sensstd_gdw, savfile, CLOBBER=clobber, CONDITIONS=conditions, $
  EXPTIME=exptime, PLOT=plot, COMPRESS=compress, $
...
;
; CALLING SEQUENCE:
;       deimos_sensstd_gdw, savfile
;
; INPUTS:
;	savfile:        IDL SAV file with spectrum and metadata
...
;
; INPUT FILES:
;       savfile = IDL SAV file from deimos_throughput.pro
;       cal_fil = standard star flux file from XIDL_DIR
;
; OUTPUT FILES:
;       outfil = output file [$DEIMOS_CALIBS/<grating>/sens_<grating>_DDMMMYYYY_NNNN.sav]
;
; PROCEDURE:
;
; EXAMPLE:
...
;       2014-Feb-18     GDW     v1.2    Convert to using SAV files
;-
;------------------------------------------------------------------------

   if  N_params() LT 1  then begin 
      print, 'Syntax - ' + $
             'deimos_sensstd_gdw, savfile [v1.2]'
      return
   endif 
   ...
   if ~ file_test(SAVFILE) then begin
      plog, logunit, 'Input file not found: '+savfile
      return
   endif

   ;; loading the SAV file will cause a struct called "extract" to spring
   ;; into being.  See doco in deimos_throughput for a description of the
   ;; contents.
   extract = read_extract_sav_file(savfile)
   ...
   outfil = subdir+'/sens_'+mgrat+ $
            '_'+meta.date+'_'+ x_padstr(meta.frame, 4, '0', /TRIM, /REVERSE)+'.sav'
   ...
   if file_test( outfil) or file_test( outfil+'.gz') then begin
      if keyword_set(CLOBBER) then begin
         ...
         plog, logunit, 'Operation would clobber existing output file '+outfil
         return
      endelse
   endif 
   ...
   if strlen(calibfil) EQ 0 then begin
      plog, logunit, 'invalid standard star based on RA/Dec -- skipping'
      return
   endif
   ...
   ;; Read in data
   spectrum = extract.spec
   
   ;; get the fiducial...
   ffile = fiducial_file( META=meta, /extract, ERRMSG=errmsg)
   if ffile eq '' then begin
      plog, logunit, errmsg
      return
   endif 
   plog, logunit, 'Fiducial file is '+ffile
   
   ;; parse the fiducial file...
   fextract = read_extract_sav_file(ffile)
   fspectrum = fextract.spec
   
   ;; shift the wavelength scale of the current spectrum to align with
   ;; the fiducial... 
   delta = coalign_spectra (fspectrum.wavelength, fspectrum.counts, $
                            spectrum.wavelength, spectrum.counts, /SHIFT)
   plog, logunit, 'Shift to align with fiducual is '+stringify(delta, '(f12.5)')+' Ang'
   ...
   if keyword_set(VERBOSE) then $
     plog, logunit, 'reading HDU 1 from cal_file '+cal_fil
   ...
   save, extract, fextract, sensstd, filename=outfil, $
         description='Generated by deimos_sensstd_gdw.pro', $
         /verbose
   plog, logunit, 'Wrote results to file '+outfil
