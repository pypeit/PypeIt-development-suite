# fit_profile Refactoring Analysis (v6)
...
- `test_fit_spectrum_failure_nan_flux` — all-NaN flux; returns
  `(False, 0.0, None, …)`.
- `test_fit_spectrum_good_matches_norm_ivar` — `good == norm_ivar.flatten() > 0`.
- `test_fit_spectrum_sn2_img_nonnegative` — `sn2_img >= 0` and correct shape.

*`_compute_bspline_knots`*:
- `test_bspline_knots_shapes` — `sigma_x.shape`, `limit > 0`,
  `min_sigma < 0 < max_sigma`, `bkpt` within bounds.
- `test_bspline_knots_bug1_fix` — `prof_nsigma=10.0` no longer crashes;
  `min_sigma=-10`, `max_sigma=10`.
- `test_bspline_knots_prof_nsigma_bounds` — parametrized over `[5, 10, 15, 20]`;
  `min/max_sigma == ±prof_nsigma`.
...
6. Add 1-D test infrastructure to `test_spatialprofile.py`.
   - Write `make_profile_inputs_1d` — calls `make_profile_inputs`, applies
     `totmask`, and returns the five pre-extracted 1-D pixel arrays
     (`spec_x`, `spat_x`, `wave_x`, `flux_x`, `ivar_x`) alongside the existing
     1-D spectral arrays (`wave`, `flux`, `fluxivar`, `trace_in`, `nspec`, `nspat`).
   - Write `_make_fsn_inputs_1d` — a helper factory (analogous to the existing
     `_make_fsn_inputs`) that feeds `_fit_spectrum_and_normalize` with 1-D
     inputs once its signature changes in step 8.
...
7. Add `spec_img` parameter to `fit_profile_refactor` and implement Block 0.
   - Add `spec_img=None` to the function signature (backward-compatible default).
   - At the top of Block 1, after constructing `totmask`, add Block 0:
     ```python
     if spec_img is None:
     ...
     ivar_x = ivar[totmask]
     ```
   - The extracted 1-D arrays are stored as local variables but not yet used
     downstream — all existing 2-D logic is unchanged at this step.
   - All 42 tests pass.

8. Convert `_fit_spectrum_and_normalize` to 1-D and update its call site and
   tests.
   - Change the signature from `(wave, flux, fluxivar, waveimg, image, ivar,
     totmask, percentile_sn2, fwhm)` to `(wave, flux, fluxivar, wave_x, flux_x,
     ivar_x, spec_x, nspec, percentile_sn2, fwhm)`.
   - Replace internal 2-D arrays: `spline_img` → `spline_img_x` (length
     `npix`), `sn2_img` → `sn2_x` (length `npix`).  Scatter operations
     `sn2_img[totmask] = sn2_interp(waveimg[totmask])` become
     `sn2_x = sn2_interp(wave_x)`.
   - Change `good = norm_ivar.flatten() > 0.0` to `good_x = norm_ivar_x > 0.0`
     (1-D bool, length `npix`).
   - Replace `xtemp` construction with:
     ```python
     cumweight = np.cumsum(row_weights)
     xtemp_x = cumweight[spec_x] / cumweight[-1]
     ```
   - Return value shapes change: `norm_obj_x`, `norm_ivar_x`, `good_x`,
     `xtemp_x`, `sn2_x` are all 1-D of length `npix`.
   - Update the call site in `fit_profile_refactor` to pass `wave_x`, `flux_x`,
     `ivar_x`, `spec_x`, `nspec` instead of the 2-D arrays.
   - Update the four `test_fit_spectrum_*` tests to use `_make_fsn_inputs_1d`:
     `assert good_x.shape == (npix,)`, `assert_array_equal(good_x, norm_ivar_x
     > 0)`, etc.
   - All 42 tests pass.

9. Convert `_compute_bspline_knots` to 1-D and update its call site and tests.
   - Add `spec_x` as a new parameter; change `dspat` from shape `(nspec, nc)` to
     shape `(npix,)`.
   - Replace `sigma_x = dspat / sigma[:, None]` with `sigma_x = dspat /
     sigma[spec_x]` (1-D).
   - Change `good` from flat boolean to 1-D boolean; replace `sigma_x.flat[good]`
     with `sigma_x[good]`.
   - Update the call site in `fit_profile_refactor`: pass `dspat_x` (1-D,
     computed as `spat_x - trace_in[spec_x]`) and `spec_x` instead of the 2-D
     `dspat`.
   - Update `_make_knots_inputs` factory and the three `test_bspline_knots_*`
     tests to pass 1-D inputs and assert `sigma_x.shape == (npix,)`.
   - All 42 tests pass.

10. Convert Blocks 6–10 in `fit_profile_refactor` to 1-D throughout.
    - **Block 6**: compute `dspat_x = spat_x - trace_in[spec_x]` (1-D); call
      `_compute_bspline_knots` with `dspat_x` and `spec_x`.  `sigma_x` is now
      1-D of length `npix`.
    - **Block 7**: replace `inside, = np.where((GOOD_PIX & IN_PIX).flatten())`
      with `inside, = np.where(GOOD_PIX & IN_PIX)` (conditions are 1-D);
      replace every `sigma_x.flat[si]`, `norm_obj_x.flat[si]`, etc. with direct
      1-D indexing `sigma_x[si]`, `norm_obj_x[si]`.  Change `mask =
      np.zeros(nspec * nc, dtype=bool)` to `mask_x = np.zeros(npix, dtype=bool)`.
    - **Block 8**: replace line 938
      (`sigma_x = dspat / sigma[:, None] - trace_corr[:, None]`) with
      `sigma_x = dspat_x / sigma[spec_x] - trace_corr[spec_x]`.
      Replace `pb = np.repeat(area, nc)[inside]` with `pb_inside =
      area[spec_x[inside]]`.  Replace `ss = sigma_x.flat[inside].argsort()`
      with `ss = sigma_x[inside].argsort()`.  Update all `.flat[]` accesses
      to direct 1-D indexing.
    - **Block 9**: unchanged (operates on 1-D `sigma` and `trace_corr`).
    - **Block 10**: replace `ss = sigma_x.flatten().argsort(kind='stable')`
      with `ss = sigma_x.argsort(kind='stable')`; replace all `.flat[ss[...]]`
      with `[ss[...]]`; replace 2-D `pb = area[:, None] * np.ones((nspec, nc))`
      with `pb_x = area[spec_x]` (1-D).
    - All end-to-end tests pass (tolerances of 10 % absorb the minor `xtemp`
      approximation).  `test_refactor_regression` will now fail at `atol=1e-10`
      because the `xtemp` change alters the B-spline trace correction
      coefficients slightly; leave it failing until step 13.
...
11. Convert `_apodize_profile` to 1-D and update its call site, fixture, and
    ...
    - Remove the `full_bsp.reshape(nspec, nc)` line in Block 13 (caller).
    - Update the `apodize_bspline` fixture: construct a 1-D `sigma_x` of length
      `npix` (e.g., flatten the existing `(nspec, nspat)` fixture array and
      recompute `ss = sigma_x.argsort(kind='stable')`); or build a smaller
      synthetic 1-D array directly.
    - Update the four `test_apodize_*` tests: `assert full_bsp.shape == (npix,)`;
      `igood = (f['sigma_x'] > f['min_sigma']) & (...)` — now 1-D bool.
    - All 42 tests pass except `test_refactor_regression` (still failing from
      step 10).

12. Add Block 13 scatter-back reconstruction.
    - After computing 1-D `full_bsp` and `pb_x = area[spec_x]`, reconstruct
      the 2-D profile:
      ```python
      profile_x = full_bsp * pb_x           # (npix,)
      profile_model = np.zeros((nspec, nc))
      profile_model[totmask] = profile_x    # scatter back
      ```
    - Row normalization (`row_sums`, `norm_safe`) operates on the 2-D
      `profile_model` exactly as before.
    - Update the `res_mode` / chi² computation to use 1-D indexing:
      `norm_obj_x[ss[inside]]`, `profile_x[ss[inside]]`,
      `norm_ivar_x[ss[inside]]`.
    - The QA call (`qa_fit_profile`) currently receives `sigma_x` (now 1-D) and
      `norm_obj` (now 1-D `norm_obj_x`); verify it still accepts 1-D inputs or
      adapt the call.
    - End-to-end tests pass (within their existing tolerances).
    - `test_refactor_regression` still fails (atol=1e-10, xtemp change).

13. Regenerate regression reference arrays and confirm all tests pass.
    - Run `python pypeit/tests/files/freeze_spatprof.py` to overwrite the three
      untracked `.npy` reference files with output from the 1-D implementation.
    - Run `pytest pypeit/tests/test_spatialprofile.py`; all 42 tests pass
      including `test_refactor_regression`.
...
14. Add `test_1d_roundtrip_matches_2d` and the six cross-implementation
    ...
    - `test_1d_roundtrip_matches_2d`: compare `profile_model` from the 1-D
      implementation against the reference arrays just regenerated, verifying
      shape and `atol=1e-8` (relaxed from `1e-10` to account for the `xtemp`
      approximation).
    - Six new comparison tests: `test_med_sn2_agreement`, `test_forced_gaussian_
      trace_agreement`, `test_bspline_row_normalization_both`, `test_profile_
      positivity_both`, `test_trace_correction_direction_both`, `test_bspline_
      fwhm_improvement_narrow`.
    - Target: 49+ tests pass.
...
The `igood`-indexed operations in lines 575–577 change analogously:
```python
sigma_x_igood = sigma_x[igood]             # same 1-D result via 2-D bool
full_bsp[igood] = yfit_out                 # same assignment via 2-D bool
```
