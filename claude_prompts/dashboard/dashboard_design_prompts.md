...
3. We need to include a few requirements related to (re)generating calibrations.  Please:

   - Consider/define how to lock the GUI when PypeIt is running, e.g. we should not allow the user to launch another PypeIt run
   - The default (only?) mode of PypeIt is to not clobber Calibration files.  We will need to implement this option and best to issue a warning check before executing it
   - I also wish for the Dashboard to include its own "status" window
   to indicate when it is waiting on a task
   - Modify the document to reflect these requirements/issues
   - Update the version number 
   - Log your work in the Logs section below
...
Questions raised while adding the (re)generation requirements (2026-06-13):

7. **Detecting an external PypeIt run (for the lock, X1).** How should the
   Dashboard know a `run_pypeit` was started *outside* it? Options I can think
   of: (a) trust `state.current_step`/`status == running` in `*_state.json`;
   (b) watch the mtime of `*_state.json` / `.log`; (c) a dedicated lock/PID file
   written by PypeIt while running. Is there an existing mechanism, or a
   preferred one to add?

It should be safe to watch the mtime of the log file. PypeIt continously writes to it while running.

8. **Clobber scope when regenerating (X2/X3).** `run_to_calibstep <step>` runs
   the step *and all preceding steps*, but reuses any existing outputs. To
   regenerate just the selected step, I'd plan to clobber only that step's
   output file(s) (preceding steps reused). Is "regenerate only the selected
   step" the right default, or should regeneration also force-rebuild the
   preceding steps?

Only clobber the selected step.
...
3. Read this doc.  Perform the 3rd task under Design document/Calibrations
...
### 2026-06-13 (Added Execution, Locking & Status requirements — v0.8)

Performed the 3rd task under "Design document / Calibrations". Added a new
cross-cutting **Execution, Locking & Status** section to
`pypeit_dashboard_design.md` (→ **v0.8**) covering the three asks:

1. **GUI locking while PypeIt runs (X1).** At most one PypeIt process at a time;
   while a run is active — including one started *outside* the Dashboard — all
   launch/(re)generate controls are disabled. Lock releases and state refreshes
   on completion. Cross-linked from C10.
2. **Clobber protection (X2/X3).** Noted that PypeIt / `run_to_calibstep` default
   to **reusing** calibration files (`reuse_calibs=True`), so a real
   regeneration must overwrite them — a destructive act that requires a
   **warning + explicit confirmation** naming the affected files, and a
   clobber/overwrite path that must be implemented (mechanism TBD).
3. **Dashboard status/activity area (X4/X5).** A dedicated indicator — separate
   from the reduction *state* — of what the Dashboard is doing and when it is
   **waiting on a task**, with a busy indicator for blocking operations (e.g.
   the state computation that may block on launch, R5), plus task-outcome
   reporting + refresh.

Also added Deferred items (XD1 queuing, XD2 cancel/abort), updated the Preamble's
planned-structure list, and posed two questions in the Calibrations
`#### Clarifications` (how to detect an external run for the lock; whether
"regenerate" should clobber only the selected step or also rebuild preceding
ones).

### 2026-06-13 (Folded in execution clarification answers — v0.9)

The user answered the two execution questions; updated
`pypeit_dashboard_design.md` (→ **v0.9**):

- **Q7 — external-run detection (X1).** Detect an active run by **watching the
  modification time of the reduction `.log` file** (PypeIt writes to it
  continuously while running). Updated the Single-run-lock bullet and X1, and
  removed the "open question" note.
- **Q8 — clobber scope (X2/X3).** Regeneration clobbers **only the selected
  step's** output file(s); preceding steps are reused, not rebuilt. Added a
  "Scope: clobber only the selected step" bullet to the clobber section and
  tightened X3.

Bumped the version + changelog.
