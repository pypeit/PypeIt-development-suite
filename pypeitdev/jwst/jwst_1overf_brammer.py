...
#def exposure_oneoverf_correction_try(file, axis=None, bpm=None, thresholds=[5, 4, 3], erode_mask=None, dilate_iterations=3,
#                                 deg_pix=64, make_plot=False, init_model=0, in_place=False, skip_miri=True, verbose=True):

def exposure_oneoverf_correction(file, rot_pypeit_fmt=True, axis=0, bpm=None, sub_background=False, thresholds=[5, 4, 3], erode_mask=None, dilate_iterations=3,
    ...
    sci = data.astype(np.float32) - init_model

    if make_plot:
        display.connect_to_ginga(raise_err=True, allow_new=True)
        display.show_image(sci, chname='sci')
        fig, ax = plt.subplots(1, 1, figsize=(6, 3))
    else:
        fig = None

    if bpm is None:
        # if one does not supply an object mask, do the object detection + background subtraction in the original code
        if sub_background:
            if deg_pix == 0:
                bw = sci.shape[0] // 64
            else:
        ...
        if make_plot:
            display.show_image(sn_mask, chname='sn_mask')
            display.show_image(mask, chname='mask')
        ...
        for _iter, thresh in enumerate(thresholds):
            ...
            if make_plot:
                ax.plot(med, alpha=0.5, label='med = np.nanmedian(sci, axis=%d)' % axis)
                ax.set_ylabel('med')
                display.show_image(sn_mask, chname='sn_mask %d' % thresh)

    else:
        gpm = np.isfinite(sci) & (sci != 0.)
        gpm &= np.invert(bpm)
        ...
        if make_plot:
            display.show_image(sci, chname='sci (masked)')
            ax.plot(med, alpha=0.5, label='med = np.nanmedian(sci, axis=%d)' % axis)
    ...
    if deg_pix == 0:
        ...
        if make_plot:
            ax.plot(np.arange(nx)[ok], cheb[ok], color='r', label='chebyshev model')
    ...
    if make_plot:
        display.show_image(model, chname='model')
        display.show_image(data-model, chname='data-model')
        #ax.plot(med-cheb, label='final model=med-cheb', zorder=0)
...
def exposure_oneoverf_correction_ori(file, axis=None, thresholds=[5, 4, 3], erode_mask=None, dilate_iterations=3,
                                 deg_pix=64, make_plot=True, init_model=0, in_place=False, skip_miri=True, verbose=True,
                                 **kwargs):
    """
    1/f correction for individual exposure
