...
def get_objmask_bkg(data, bpm=None, error=None, brightstar_nsigma=2, npixels=4, fwhm=3, back_type='sextractor',
    ...
    while iiter < 2:
        ...
        log.info('Making bright star mask with iter={:}'.format(niter+1))
...
def get_rowamp_model(data_masked, namp, minimum_pixels=10, rej_nsigma=3, maxiters=5, evenOdd=True, inst='NIRCam', show=False, skipcol_show=False):
    ...
    if namp == 4:
        ...
        if inst == 'NIRCam':
            ...
            raise PypeItError('Only NIRCam is supported at this moment.')
        ...
        if show:
            ...
            if np.ndim(slowread_model) == 2:
                ...
                log.info('slowread model is a float with value {:}'.format(slowread_model))
            display.show_image(fastread_model, chname='fastread model%s' % suffix)
    else:
        raise PypeItError('{:} amplifiers is not implemented yet.'.format(namp))
...
def BKG2D(data, back_size, bpm=None, filter_size=(3, 3), sigclip=5, back_type='sextractor', back_rms_type='biweight',
    ...
    if 'median' in back_type.lower():
        ...
        log.warning('{:} Background is not found, using MedianBackground Instead.'.format(back_type))
        back_type = 'median'
        bkg_estimator = MedianBackground()
    ...
    if back_rms_type.lower() == 'std':
        ...
        log.warning('{:} Background RMS type is not found, using STD Instead.'.format(back_rms_type))
        bkgrms_estimator = StdBackgroundRMS()

    if verbose:
        log.info('Estimating {:} BACKGROUND with Photutils Background2D.'.format(back_type))
